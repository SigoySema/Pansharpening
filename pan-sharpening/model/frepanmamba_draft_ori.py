...
class Net(nn.Module):
    def __init__(self, 
        ...
        # 序列化 -> B,N,C
        self.ms_to_token = PatchEmbed(in_chans=ms_channels, embed_dim=self.embed_dim, patch_size=self.patch_size, stride=self.stride)
        self.pan_to_token = PatchEmbed(in_chans=pan_channels, embed_dim=self.embed_dim, patch_size=self.patch_size, stride=self.stride)

        # MS和PAN的特征提取（VSSBlock） - 每个阶段有 4 个 VSSBlock
        self.ms_feature_extraction_level = nn.ModuleList([VSSBlock(hidden_dim=int(self.embed_dim), drop_path=self.drop_path_rate, norm_layer=nn.LayerNorm, attn_drop_rate=0, expand=self.mlp_ratio, d_state=int(self.base_d_state)) for _ in range(4)])
        self.pan_feature_extraction_level = nn.ModuleList([VSSBlock(hidden_dim=int(self.embed_dim), drop_path=self.drop_path_rate, norm_layer=nn.LayerNorm, attn_drop_rate=0, expand=self.mlp_ratio, d_state=int(self.base_d_state)) for _ in range(4)])

        # CMF_Mamba模块
        self.CMF_Mamaba_level0 = CMF_Mamba(self.embed_dim)
        self.CMF_Mamaba_level1 = CMF_Mamba(self.embed_dim)
        self.CMF_Mamaba_level2 = CMF_Mamba(self.embed_dim)
        self.CMF_Mamaba_level3 = CMF_Mamba(self.embed_dim)
    ...
    def forward(self, ms, _, pan):
        ...
        # MS特征提取
        for i in range(4):  # 现在每一层有 4 个 VSSBlock
            ms_f = self.ms_feature_extraction_level[i](ms_f, [h, w])

        # PAN特征提取
        for i in range(4):  # 现在每一层有 4 个 VSSBlock
            pan_f = self.pan_feature_extraction_level[i](pan_f, [h, w])

        # 初始化 cmf_f_out 为零
        cmf_f_out = torch.zeros_like(ms_f)  # 或者使用相同形状的零张量

        # CMF_Mamba融合
        cmf_f_level0 = self.CMF_Mamaba_level0(ms_f, pan_f)
        cmf_f_out += cmf_f_level0  # 第一层融合

        # 对于更高层次（Level 1-3），重复相同的操作
        for level in range(1, 4):
            ms_f = self.ms_feature_extraction_level[level](ms_f, [h, w])
            pan_f = self.pan_feature_extraction_level[level](pan_f, [h, w])
...
#     def _forward(self, input: torch.Tensor, x_size):
...
#         Yl, Yh = xfm(prepare)
#         h00 = torch.zeros(prepare.shape).float()
...
#         else:
#             return self._forward(input, x_size)
