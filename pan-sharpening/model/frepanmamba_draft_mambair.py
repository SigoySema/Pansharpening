...
class Net(nn.Module):
    ...
    def _create_feature_extraction_layers(self):
        ...
        self.ms_feature_extraction_level = nn.ModuleList([
            ...
            MambaIR(embed_dim = int(self.embed_dim),img_size=128,in_chans=4,depths=(1, 1, 1, 1, 1, 1),upscale=1)
        ...
        self.pan_feature_extraction_level = nn.ModuleList([
            ...
            #          expand=self.mlp_ratio, 
            #          d_state=int(self.base_d_state)) 
            MambaIR(embed_dim = int(self.embed_dim),img_size=128,in_chans=1,depths=(1, 1, 1, 1, 1, 1),upscale=1)
            for _ in range(num_blocks)
        ])
